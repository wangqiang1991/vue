<template>
  <div class="app-container calendar-list-container" style="padding-top:10px;">
    ...
    <div class="filter-container">
      ...
      <el-input @keyup.enter.native="listdoSearch()" v-model="listQuery.activityTitle" style="width: 180px;margin:0 10px;" placeholder="活动标题"></el-input>   
    ...
    <!-- 保存首页设置 -->
    <el-dialog title="新建活动推送" :visible.sync="addDialogFormVisible">

      <el-form :model="form">  

        <el-form-item label="指定推送活动">
          <el-button type="text" @click="showSelectSpace()" size="small">选择活动</el-button>
          ...
          <el-table :data="spaceList" style="width: 100%" border>
            ...
            <el-table-column label="活动名称"  align="center" width="250">
              <template scope="scope">
                <span >{{scope.row.title}}</span>
              </template>
            </el-table-column>

            <el-table-column label="活动描述"  align="center" width="250">
              <template scope="scope">
                <span >{{scope.row.description}}</span>
              </template>
            </el-table-column>
        ...
        <uploadImage v-show='show' style='margin-bottom:22px;padding-left:68px;' :id="'planUpload1'" :width="275.7" :height="140" ref="planUpload" @uploadSuccess="uploadSuccess"></uploadImage>
    ...
    <!-- 选择空间 -->
    <el-dialog title="选择推送活动" :visible.sync="selectDialogFormVisible">

      <div class="filter-container">
        <el-input @keyup.enter.native="doSearch()" v-model="spaceListQuery.title" style="width: 200px;" class="filter-item" placeholder="活动名称">
        </el-input>
  ...
  export default {
    ...
    methods: {
      ...
      findAll() {
        ...
        params.activityTitle = this.listQuery.activityTitle;

        this.http.get(this).url(this.config.propel.activity_list).params(params).request(function (response) {
      ...
      onSelectedSpace(item) {
        ...
        this.homeTop.objectId = item.activityId;
      ...
      findByPage() {
        ...
        this.http.get(this).url(this.config.softWear.activities).params(params).request(function(response) {
      ...
      save() {
        ...
        if (!this.homeTop.objectId || !this.homeTop.objectId.trim()) {
          this.$message({type: 'warning',message: '请指定推送活动'});
          return;
        }
