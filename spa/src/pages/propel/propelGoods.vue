<template>
  <div class="app-container calendar-list-container" style="padding-top:10px;">
    ...
    <div class="filter-container">
      ...
      <el-input @keyup.enter.native="listdoSearch()" v-model="listQuery.goodsTitle" style="width: 180px;margin:0 10px;" placeholder="商品标题"></el-input>  
      <el-input @keyup.enter.native="listdoSearch()" v-model="listQuery.goodsNumber" style="width: 180px;margin:0 10px 0 0;" placeholder="商品编码"></el-input>   
    ...
    <el-table           
      ...
      <el-table-column width="200" align="center" label="封面">
        <template scope="scope">
          <img style="width:50px;height:50px;margin-top: 10px;" :src="scope.row.cover+'?imageView2/1/w/512/h/512'">
        </template>
      </el-table-column>
    ...
    <!-- 保存首页设置 -->
    <el-dialog title="新建商品推送" :visible.sync="addDialogFormVisible">

      <el-form :model="form">  

        <el-form-item label="指定推送商品">
          ...
          <el-table :data="spaceList" style="width: 100%" border>
            ...
            <el-table-column label="商品编码"  align="center" width="250">
              <template scope="scope">
                <span >{{scope.row.goodsNumber}}</span>
              </template>
            </el-table-column>
        ...
        <p v-show='show' style="margin-bottom:5px;margin-top:0;"><span>推送封面 </span><span style="color:#FF4949;margin-left:10px;" type="text">建议尺寸640*640</span></p>
        <uploadImage v-show='show'  style='margin-bottom:22px;padding-left:68px;' :id="'planUpload'" :width="140" :height="140" ref="planUpload" @uploadSuccess="uploadSuccess"></uploadImage>
    ...
    <!-- 选择空间 -->
    <el-dialog title="选择推送商品" :visible.sync="selectDialogFormVisible">

      <div class="filter-container">
        <el-input @keyup.enter.native="doSearch()" v-model="spaceListQuery.keyword" style="width: 200px;" class="filter-item" placeholder="商品名称">
        </el-input>
  ...
  export default {
    ...
    methods: {
      ...
      findAll() {
        ...
        params.goodsTitle =  this.listQuery.goodsTitle;

        this.http.get(this).url(this.config.propel.goods_list).params(params).request(function (response) {
      ...
      onSelectedSpace(item) {
        ...
        this.homeTop.goodsNumber = item.goodsNumber;
      ...
      findByPage() {
        ...
        params.keyword = this.spaceListQuery.keyword;
        params.goodsStatus = 3;
        this.http.get(this).url(this.config.goods.list_url_v2).params(params).request(function(response) {
      ...
      save() {
        ...
        if (!this.homeTop.objectId || !this.homeTop.objectId.trim()) {
          this.$message({type: 'warning',message: '请指定推送商品'});
          return;
        }
