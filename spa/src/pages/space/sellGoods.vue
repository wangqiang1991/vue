    ...
    <el-table :data="dataList" v-loading="loading" element-loading-text="努力加载中..." border fit highlight-current-row
      ...
      <el-table-column prop="goods" label="商品封面" align="center" vertical="middle" width="120">
        <template scope="scope">
          <img style="width:50px;height:50px;margin-top: 10px;" :src="scope.row.cover+config.imgtmb._200"
      ...
      <el-table-column
        ...
        align="center">
        <template scope="scope">
          <span v-if="scope.row.putaway">已上架</span>
          <span style="color: red" v-else="scope.row.putaway">已下架</span>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      findAllGoods() {
        this.loading = true;
        this.http.get(this).url(config.space.goods_url).restful({
