    ...
    <el-table-column type="expand" label="展开">
      <template scope="props" >
        ...
        <p class="goods_style_message">
          <span>总收入: <b>￥{{(props.row.saleMoney/ 100).toFixed(2)}}</b></span>
          <span>总成本: <b>￥{{(props.row.saleCostMoney/ 100).toFixed(2)}}</b></span>
          <span>总利润: <b>￥{{(props.row.saleProfit/ 100).toFixed(2)}}</b></span>
          <span>总利润率: <b>{{(props.row.saleProfitRatio *100).toFixed(2)}}%</b></span>
        </p>
  ...
  export default {
    ...
    methods:{
      ...
      findByPage() {
        ...
        this.http.get(this).url(this.config.statistics.goodsSalesStorageStatistics_items).params(params).request((response) => {
