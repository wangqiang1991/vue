<template>
  <div class="app-container calendar-list-container">
    ...
    <div class="filter-container">
      <el-input @keyup.enter.native="doSearch()" v-model="listQuery.nickName" style="width: 200px;" class="filter-item"
                placeholder="输入设计师昵称">
      </el-input>

       <el-input @keyup.enter.native="doSearch()" v-model="listQuery.phone" style="width: 200px;" class="filter-item"
                placeholder="输入设计师电话">
      </el-input>

      <el-select @change="doSearch" class="filter-item" v-model="listQuery.type" clearable placeholder="设计师类型">
    ...
    <el-dialog :title="title" :visible.sync="addDialogFormVisible">
      <el-form class="small-space" :model="systemUserDetail" label-position="left" label-width="100px"
        ...
        <el-form-item label="昵称">
          <el-input v-model="designer.nickName" placeholder="请输入设计师昵称(必填)" maxlength="50"></el-input>
        </el-form-item>

        <el-form-item label="头像"><span style="color:#ff4949;">建议为正方形640*640</span>
          <el-progress v-show="uploading" type="circle" :percentage="percentage"></el-progress>
          <div v-show="designer.head && !uploading" class="el-upload el-upload--picture-card" style="margin-right:0;display:block;">
            <img id="uploadCover" title="点击修改图片" style="width:100%; height:100%"
                 :src="designer.head + config.imgtmb._200">
          </div>
          <div v-show="!designer.head && !uploading" style="display:block; margin-left:0;">
        ...
        <el-form-item label="真实姓名">
          <el-input v-model="designer.reallyName" placeholder="设计师真实姓名" maxlength="50"></el-input>
        </el-form-item>

        <el-form-item label="手机">
          <el-input v-model="designer.phone" placeholder="联系电话" maxlength="12"></el-input>
        </el-form-item>

        <el-form-item label="邮箱">
          <el-input v-model="designer.email" placeholder="联系邮箱" maxlength="50"></el-input>
        </el-form-item>

        <el-form-item label="个人主页">
          <el-input v-model="designer.homePage" placeholder="个人主页" maxlength="50"></el-input>
        </el-form-item>
        ...
        <el-form-item label="擅长领域">
          ...
          <el-input class="input-new-tag" v-if="inputVisible" style="width:20%;" v-model="inputValue" ref="saveTagInput" size="small"
        ...
        <el-form-item label="设计师简介">
          <el-input type="textarea" :autosize="{ minRows: 3, maxRows: 4}" maxlength="255" placeholder="请填写设计师简介"
  ...
  export default {
    ...
    methods: {
      ...
      findByPage() {
        ...
        this.http.get(this).url(this.config.designer.list_url).params(this.listQuery).request(
          (response) => {
      ...
      save() {
        ...
        if (!this.designer.nickName || !this.designer.nickName.trim()) {
          this.$message({message: '设计师昵称不能为空', type: 'warning'});
          return;
        }
        ...
        if (!this.designer.reallyName || !this.designer.reallyName.trim()) {
          this.$message({message: '请输入设计师真实姓名', type: 'warning'});
          return;
        }
        if (!this.designer.phone || !this.designer.phone.trim()) {
          this.$message({message: '请输入设计师手机', type: 'warning'});
          return;
        }
        this.designer.tag = this.designer.tags.join(",");
        if (!this.designer.tag.length) {
          this.$message({message: '请输入设计师的擅长领域', type: 'warning'});
          return;
        }
