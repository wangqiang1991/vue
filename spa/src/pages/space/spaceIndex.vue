<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :data="dataList" v-loading.body="listQuery.loading" border fit highlight-current-row style="width: 100%" id="spaceTable">
      ...
      <el-table-column width="200" align="center" label="封面">
        <template scope="scope">
          <img style="width:110px;height:65px;margin-top: 10px;" :src="scope.row.cover+'?imageView2/1/w/512/h/305'" class="gc_praiselist_list_img">
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="addDialogFormVisible">
      <el-form :model="form">
        ...
        <!-- <el-form-item label="首页封面:" :label-width="formLabelWidth">
          <span style="color:#FF4949" type="text">建议尺寸:1024*610</span>
        </el-form-item>
        ...
        <el-form-item label="" :label-width="formLabelWidth">
          ...
          <div v-show="homeTop.cover && !uploading" class="el-upload el-upload--picture-card" style="margin-left:23px;">
            <img id="uploadCover1" title="点击修改图片" style="width:100%; height:100%" :src="homeTop.cover+config.imgtmb._200">
          </div>
          <div v-show="!homeTop.cover && !uploading" style="display:inline-block; margin-left:23px">
            <div class="el-upload el-upload--picture-card" id="uploadCoverBtn1">
              <i data-v-5b37d0d4="" class="el-icon-plus"></i>
            </div>
          </div>
        </el-form-item> -->

        <el-form-item label="指定空间">
          <el-button type="text" @click="showSelectSpace()" size="small">选择空间</el-button>
          <el-table :data="spaceList" style="width: 100%" border>

            <el-table-column label="空间封面"  align="center" width="150">
              <template scope="scope">
                <img :src="scope.row.cover" style="margin-top:6px;width:110px;height:65px;">
              </template>
            </el-table-column>

            <el-table-column prop="title" label="空间名称">
            </el-table-column>
    ...
    <!-- 选择空间 -->
    <el-dialog title="选择空间" :visible.sync="selectDialogFormVisible">

      <div class="filter-container">
        <el-input @keyup.enter.native="doSearch()" v-model="spaceListQuery.keyword" style="width: 200px;" class="filter-item" placeholder="空间名称">
        </el-input>
      ...
      <div class="gc-praiselist_content_list">
        <el-table v-loading="spaceListQuery.loading" element-loading-text="努力加载中..." ref="multipleTable" :data="spaceDataList" border style="width: 100%" @sort-change="sortChange">
          ...
          <el-table-column label="空间封面"  align="center">
            <template scope="scope">
              <img :src="scope.row.cover" style="margin-top:6px;width:110px;height:65px;">
            </template>
          </el-table-column>
  ...
  export default {
    ...
    methods: {
      findAll() {
        ...
        this.http.get(this).url(config.home.home_top_space_list).params({}).request(function (response) {
      ...
      setSort() {
        const el = document.querySelectorAll('#spaceTable>.el-table__body-wrapper > table > tbody')[0];
      ...
      onSelectedSpace(item) {
        ...
        this.homeTop.objectId = item.spaceId;
      ...
      save() {
        ...
        if (!this.homeTop.title || !this.homeTop.title.trim()) {
          this.$message({type: 'warning',message: '请选择首页空间'});
          return;
        }
