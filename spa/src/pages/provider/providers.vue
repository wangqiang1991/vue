<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <el-select @change="doSearch()" clearable style="width: 130px" class="filter-item" v-model="listQuery.condition_way" placeholder="供应商名称">
        <el-option v-for="item in search_ways" :key="item.key" :label="item.display_name" :value="item.key">
        </el-option>
      </el-select>

      <el-input @keyup.enter.native="doSearch()" v-model="listQuery.condition" style="width: 200px;" class="filter-item" :placeholder="listQuery.condition_way == 'code' ? '供应商编码' : '供应商名称'">
      </el-input>
    ...
    <el-table :data="dataList" v-loading="listQuery.loading" element-loading-text="努力加载中..." border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column label="供应商名称">
        <template scope="scope">
          <span @click="handleEdit(scope.row)" class="link-type">{{scope.row.providerName}}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="供应商编码" width="200">
        <template scope="scope">
          <span>{{scope.row.providerCode}}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="经营性质" width="200">
        <template scope="scope">
  ...
  export default {
    ...
    methods: {
      ...
      findByPage() {
        ...
        if (this.listQuery.condition_way == 'code') {
          params.providerCode = this.listQuery.condition;
        } else {
          params.providerName = this.listQuery.condition;
        }
