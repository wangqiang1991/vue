	...
	$.Zoom = function(element, options) {
		...
		zoom.onPinch = function(e) {
			...
			if (scale > zoom.options.maxZoom) {
				scale = 3//zoom.options.maxZoom - 1 + Math.pow((scale - zoom.options.maxZoom + 1), 0.5);
			}
			if (scale < zoom.options.minZoom) {
				scale = 1//zoom.options.minZoom + 1 - Math.pow((zoom.options.minZoom - scale + 1), 0.5);
			}
