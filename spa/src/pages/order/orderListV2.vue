<template>
  <div class="app-container calendar-list-container">
    <div v-show="state=='list'" class="filter-container">
      ...
      <el-input @keyup.enter.native="doSearch()" v-model="listQuery.orderNumber" style="width: 205px;"
                class="filter-item" :placeholder="(orderType == 1 || orderType == 4) ? '订单编号(主订单号)' : '订单编号(主订单号|子订单号)' ">
      </el-input>

      <el-input @keyup.enter.native="doSearch()" v-model="listQuery.reallyName" style="width: 150px;"
                class="filter-item" placeholder="收货人姓名">
      </el-input>
      <el-input @keyup.enter.native="doSearch()" maxlength='11' v-model="listQuery.phone" style="width: 150px;" class="filter-item"
                placeholder="收货人手机号码">
      </el-input>
      ...
      <el-badge :value="choiceDatas.length" class="item">
        <el-button @click="exportOrders" :disabled="!choiceDatas.length" class="filter-item fontFamily icon-print" style="margin-left: 10px;" type="primary"  v-if="orderType == 2">打印销售单</el-button>
      </el-badge>
<!-- 
      <el-badge :value="choiceDatas.length" class="item">
        <el-button @click="exportDelivery" :disabled="!choiceDatas.length" class="filter-item fontFamily icon-print" style="margin-left: 10px;" type="primary"  v-if="orderType == 2">打印发货单</el-button>
      </el-badge> -->
    ...
    <el-table v-show="state=='list'" :data="dataList" v-loading="loading" element-loading-text="努力加载中..." border fit
      ...
      <el-table-column align="center" label="主订单编号" >
      ...
      <el-table-column align="center" v-if="orderType == 1 || orderType == 4" label="下单时间" >
        <template scope="scope">
          <span>{{scope.row.createdAt | parseTime('{y}-{m}-{d} {h}:{i}')}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" v-else label="支付时间" >
        <template scope="scope">
      ...
      <el-table-column label="打印选择" align="center"   v-if="orderType == 2">
        <template scope="scope">
  ...
  export default {
    ...
    methods: {
      ...
      findByPage() {
        ...
        if (this.orderType == 6) {
          this.listQuery.queryTransactionAbort = true;
          this.listQuery.queryType = null;
        } else {
          this.listQuery.orderStatus = this.orderType;
        }
      ...
      showDetail(order) {
        ...
        this.http.get(this).url(config.order.order_detail_url).restful({
          ...
          if (response.data.code == 0) {
            ...
            response.data.data.orderListType = this.orderType;
      ...
      exportOrders() {
       ...
       if(process.env.NODE_ENV == "production") {
          //线上url
           url = "/api/make-pdf/create-pdf-delivery/构巢销售单.pdf";
        } else if(process.env.NODE_ENV == "test") {
          //测试
           url = "/api/make-pdf/create-pdf-delivery/构巢销售单.pdf";
        } else {
          //本地url
          url = "http://192.168.20.104:8888/api/make-pdf/create-pdf-delivery/构巢销售单.pdf";
        }
      ...
      exportDelivery() {
        ...
        if(process.env.NODE_ENV == "production") {
          //线上url
           url =  "/api/make-pdf/create-pdf-sent/构巢订单发货单.pdf";
        } else if(process.env.NODE_ENV == "test") {
          //测试
           url =  "/api/make-pdf/create-pdf-sent/构巢订单发货单.pdf";
        } else {
          //本地url
          url = "http://192.168.20.104:8888/api/make-pdf/create-pdf-sent/构巢订单发货单.pdf";
        }
