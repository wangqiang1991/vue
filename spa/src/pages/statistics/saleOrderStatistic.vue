    ...
    <div id="gmvData_vue" class="app-container calendar-list-container">
      ...
      <div class="gc-praiselist_content_list chart_box" style="margin-top:15px;">
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick">
          <el-tab-pane label="付款订单总量" name="1">
            <div id="chart1" class="chart_div"></div>
          </el-tab-pane>
          <el-tab-pane label="付款人数" name="2">
            <div id="chart2"  class="chart_div"></div>
          </el-tab-pane>
          <el-tab-pane label="付款订单总收入" name="3">
            <div id="chart3"  class="chart_div"></div>
          </el-tab-pane>
          <el-tab-pane label="付款订单总毛利润" name="4">
  ...
  export default {
    ...
    methods:{
      ...
      findByDetail() {
        ...
        this.http.get(this).url(this.config.statistics.sale_order_detail).params(params).request((response) => {
          if (response.data.code != 0) {
            this.$message({message: response.data.message ? response.data.message : "系统错误", type: 'error'});
            return;
          }
      ...
      printOrderDetail() {
        ...
        if(process.env.NODE_ENV == "production") {
          //线上url
           url =  "/api/statistics-pdf/create-statistics/实际销售订单明细.pdf";
        } else if(process.env.NODE_ENV == "test") {
          //测试
           url =  "/api/statistics-pdf/create-statistics/实际销售订单明细.pdf";
        } else {
          //本地url
          url = "http://192.168.20.104:8888/api/statistics-pdf/create-statistics/实际销售订单明细.pdf";
        }
        ...
        if (session.isLogin()) {
          ...
          url = url + "?access_token=" + user.accessToken + "&beginDate=" + beginDate + "&endDate=" + endDate + "&kind=" + 2;
